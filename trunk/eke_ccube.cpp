...
int main()
{
  ...
  while(1)
	...
	cout << npm << endl;
      if((delta_functional<0)&&(-delta_functional<1e-10))
	break;
    }
  ...
  for(int i=0;i<nx;++i)
    for(int j=0;j<ny;++j)
      for(int k=0;k<nz;++k)
	if(point_type(i,j,k)==1)
	  {
	    ef_test(i,j,k)=coordinates(i,j,k);
	    ef_test(i,j,k)*=-.25/M_PI/pow(norm(coordinates(i,j,k)),3);
		     ...
		     const double & rs)
{
  for(int i=0;i<nx;++i)
    for(int j=0;j<ny;++j)
      for(int k=0;k<nz;++k)
	{
	  coordinates(i,j,k)=TinyVector<double,3>(-.5*lx+i*lx/nx,-.5*ly+j*ly/ny,-.5*lz+k*lz/nz);
	  if (norm(coordinates(i,j,k))>=rs)
	    point_type(i,j,k)=1;
	  else
	    point_type(i,j,k)=2;  
	}
			...
			const double & rs,const double & charge)
{
  ...
  for(int n=0;n<np;++n)
	...
	for(int j=0;j<2;++j)
      ...
      point_type(ind_np_x,ind_np_y,ind_np_z)=3;     
    }
  
  //Put negative charges at the cube boundaries
  for(int n=0;n<np;++n)
    { 
      DiscreteUniform<int> drg(3);
      DiscreteUniform<int> rgx(nx);
      DiscreteUniform<int> rgy(ny);
      DiscreteUniform<int> rgz(nz);
      int plane=drg.random();
      if(plane==0)
	{
	  int ni=rgx.random();
	  int nj=rgy.random();
	  charge_density(ni,nj,nz-1)+=-delta_charge;
	}
      else if(plane==1)
	{
	  int nj=rgy.random();
	  int nk=rgz.random();
	  charge_density(nx-1,nj,nk)+=-delta_charge;
	}
      else if(plane==2)
	{
	  int ni=rgx.random();
	  int nk=rgz.random();
	  charge_density(ni,ny-1,nk)+=-delta_charge;
	}
...
double loop_move(Array<TinyVector<double,3>,3> & electric_field,
		 const int & nx,const int & ny,const int & nz,
		 const double & lx,const double & ly,const double & lz)
{
  //Choose a node
